...
from pages import EpicGridPage, HomePage
...
class TestNavigationHierarchy:
    ...
    def test_home_page_content_visible(self) -> None:
        """HomePage sections are visible after authentication."""
        home_page = HomePage(self.page)
        home_page.expect_home_sections_visible()

        assert self.page.locator(home_page.ALERT_BANNER).count() == 0, (
...
class TestPageObjectInheritance:
    ...
    def test_epic_page_has_all_parent_methods(self, page: Page) -> None:
        ...
        # EpicGridPage-specific methods
        assert hasattr(epic_page, "navigate_to_backlog")
        assert hasattr(epic_page, "quick_add_epic")

    def test_home_page_has_header_but_no_sidebar(self, page: Page) -> None:
